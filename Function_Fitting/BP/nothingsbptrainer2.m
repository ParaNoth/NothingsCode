function [ wk,wj,bk,bj,jt ] = nothingsbptrainer2( Xp,Dp,nh,lr,e,maxiter )
%此函数为一层隐层的bp网络训练函数，网络函数均为sigmoid函数。输入Xp为样本矩阵，Dp为教师矩阵，nh为中间层神经元个数，lr为学习率，e为误差限，maxiter为最大迭代次数。输出wk为上层的权重矩阵，wj为下层的权重矩阵，jt为误差变化曲线。
%   此处显示详细说明
...
for i = 1:maxiter
    ...
    for j = 1:x_num
        ...
        in2 = out1*wk;
        out2 = 1./(1+exp(-in2));
        errorOut = (Dp(j,:)-out2).*out2.*(1-out2);
